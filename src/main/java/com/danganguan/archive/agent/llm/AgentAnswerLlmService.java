...
public class AgentAnswerLlmService {
    ...
    private String systemPrompt() {
        return """
                ...
                6. 默认不超过 160 字；用户没有要求展开时，不要长篇解释。
                7. 统计类问题优先直接给数字；检索类问题最多列 5 个最相关档案。
                8. 如果工具结果不能证明某个条件成立，直接说“当前范围未检索到明确证据”，不要把无关档案列成候选。
                9. 可以说明“题名显示了学生姓名/档号”，但不要根据档号、目录名或文件名推断未明示的档案内容。
                10. 区分证据等级：METADATA 只能说明题名/目录/基础摘要命中；CONTENT 才能支撑正文内容、材料类型、导师等结论。
                """;
    }
    ...
    private String userPrompt(String userMessage,
        ...
        return limit("""
                ...
                输出风格：简短直接，先给结论，再给必要依据。请基于以上内容生成最终回答。
                """.formatted(
