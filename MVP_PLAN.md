...
处理后的文件可通过工作区或正式档案文件接口预览，默认浏览器内联打开，追加 `download=true` 可下载。
...
1. 处理后先做文档分析，再进入命名。
2. PDF 默认先提取文本层；PNG/图片在外部模型模式下会作为视觉输入。
3. 给定文件命名示例时，系统优先解析示例格式，而不是让模型自由发挥。
4. 示例 `N2007-JX12•11•21-1韩学敏.pdf` 会被解析为固定前缀、序号、姓名三段。
5. 识别到姓名 `尚宇` 且当前序号为 14 时，会稳定生成 `N2007-JX12•11•21-14尚宇`。
6. 如果原始文件名已经符合该格式，会优先保留原始规范名。

大量同级文件参考尚未正式接入；当前序号来自同一任务下已生成的工作区档案数量。后续需要增加命名历史/同级文件召回，再由规则层确定前缀和下一序号。

## 第九阶段目标

文档分析链路调整为围绕原始上传文件，而不是处理后的目标格式：

1. PDF 源文件先抽文本层。
2. 图片源文件先走本地 OCR。
3. 扫描型 PDF 抽不到文本时，先渲染前几页并走本地 OCR。
4. OCR 文本不足时，外部模型模式再发送原图或渲染页做视觉兜底。
5. ZIP 会解压内部 PDF/图片后按原始材料分析。
6. OCR Provider 可配置为 `none`、`tesseract` 或 `python`。

Python OCR 可作为外部脚本接入，不嵌入 Java 进程；当前示例脚本为 `scripts/ocr/pytesseract_ocr.py`。
